@php
    $projects = [
        ...
        [
            ...
            'badge' => 'Live',
            'badge_color' => 'from-blue-600 to-sky-500',
            ...
            >
                ...
                @if (!empty($project['images']))
                    ...
                    @if ($imgCount === 2)
                        <div class="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                            @foreach ($project['images'] as $projectImage)
                                <button
                                    type="button"
                                    class="project-gallery-thumb group/img relative flex min-h-[200px] cursor-zoom-in items-center justify-center overflow-hidden rounded-xl bg-slate-100 ring-1 ring-blue-200/70 transition hover:ring-blue-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"
                    ...
                    @else
                        <button
                            type="button"
                            class="project-gallery-thumb mb-3 flex w-full cursor-zoom-in items-center justify-center overflow-hidden rounded-xl bg-slate-100 ring-1 ring-blue-200/70 transition hover:ring-blue-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"
                            data-project-lightbox
                        ...
                        @if ($imgCount > 1)
                            <div class="mb-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
                                @foreach (array_slice($project['images'], 1, 6) as $projectImage)
                                    <button
                                        type="button"
                                        class="project-gallery-thumb flex min-h-[120px] cursor-zoom-in items-center justify-center overflow-hidden rounded-lg bg-slate-100 ring-1 ring-blue-200/70 transition hover:ring-blue-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"
                                        data-project-lightbox
                                        data-project-lightbox-src="{{ asset($projectImage) }}"
                                        data-project-lightbox-alt="{{ $project['title'] }} — screenshot"
                                    >
                    ...
                    <p class="mb-1 text-[0.7rem] text-slate-500">
                ...
                <div class="mb-2 flex items-center justify-between text-xs">
                    <span class="inline-flex items-center gap-1 rounded-full bg-slate-950 px-2 py-0.5 text-[0.65rem] font-medium text-white">
                ...
                {{-- DESCRIZIONE --}}
                <p class="mt-2 text-[0.95rem] leading-relaxed text-slate-600">
                    {{ $project['description'] }}
                </p>
                ...
                <div class="mt-3 flex flex-wrap gap-2">
                    @foreach ($project['stack'] as $tech)
                        <span class="rounded-full bg-blue-50 px-2 py-0.5 text-[0.65rem] text-slate-800 ring-1 ring-blue-200/90">
                            {{ $tech }}
                        </span>
                    @endforeach
                </div>
                ...
                @if ($project['link'])
                    <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        class="mt-4 inline-flex items-center text-xs font-medium text-blue-800 hover:text-slate-900"
                    >
                        {{ $project['link_label'] }}
